...
static void blk_stat_timer_fn(struct timer_list *t)
{
	...
	for_each_online_cpu(cpu) {
		...
		struct blk_rq_stat *percpu_stat; // EHP

		cpu_stat = per_cpu_ptr(cb->cpu_stat, cpu);
		percpu_stat = per_cpu_ptr(cb->percpu_stat, cpu); //EHP
		
		for (bucket = 0; bucket < cb->buckets; bucket++) {
			if (cpu_stat[bucket].nr_samples >= 1) {
				percpu_stat[bucket].nr_samples = cpu_stat[bucket].nr_samples;
				percpu_stat[bucket].min = cpu_stat[bucket].min;
				percpu_stat[bucket].max = cpu_stat[bucket].max;
				percpu_stat[bucket].mean = div_u64(cpu_stat[bucket].batch, 
			...
			unsigned int buckets, void *data)
{
	...
	cb->percpu_stat = __alloc_percpu(buckets * sizeof(struct blk_rq_stat),
					__alignof__(struct blk_rq_stat)); //EHP
			   ...
			   struct blk_stat_callback *cb)
{
	...
	for_each_possible_cpu(cpu) {
		...
		for (bucket = 0; bucket < cb->buckets; bucket++){
			blk_rq_stat_init(&cpu_stat[bucket]);
			blk_rq_stat_init(&percpu_stat[bucket]); //EHP
		}
	}
