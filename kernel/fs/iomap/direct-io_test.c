...
*/
        ...
        if ((dio->iocb->ki_flags & IOCB_HIPRI) && (irq_poll_switch != NULL)){
                ...
                if(++sc->ioctr == 100000) {
                        sc->ioctr = 0;
                        printk("polled io: %llu, int io: %llu", sc->pollctr, sc->intctr);
			//sc->pollctr = 0; sc->intctr = 0;
		}
		sc->counter++;
		if(sc->poll == true && sc->counter >= sc->param1) { // param1: # of polled I/Os
			sc->poll = !(sc->poll);
			sc->counter = 0;
		}
		else if(sc->counter >= sc->param2) { // param2: # of interrupt I/Os
			sc->poll = !(sc->poll);
			sc->counter = 0;
		}
                ...
                if (sc->poll){
                        ...
                        //if(sc->counter[bucket] >= 1000){
                        if(0 && sc->counter >= 1000){
                                printk("return to poll");
                                //sc->counter[bucket] = 0;
                                sc->counter = 0;
