...
static int simple_iomap_bucket(struct bio* bio)
{
	int ddir, sectors, bucket;

	sectors = bio->bi_iter.bi_size / 512;
...
/*	if (dio->iocb->ki_flags & IOCB_HIPRI) {
	...
	if(irq_poll_switch != NULL)
		sc = per_cpu_ptr(irq_poll_switch, cpu_num);

	if ((dio->iocb->ki_flags & IOCB_HIPRI) && (irq_poll_switch != NULL)){
		sc->ioctr++;

		if (sc->mode) { /* for all polled I/Os (INT: 0, CP: 1, PAS: 2, Overloaded: 3) */
			bio_set_polled(bio, dio->iocb);
			dio->submit.poll_bio = bio;
			if(sc->mode == 1) { 
				sc->cp_cnt++;
			...
			if(sc->mode == 4) { // EHP
				bucket = simple_iomap_bucket(bio);
				//printk("bucket in submit %d\n",bucket);
				if(sc->ehpmode[bucket] != 4){
				//	printk("ehp int mode");
				...
				//printk("ehp poll mode");
				//sc->pas_cnt++;
				//sc->pas_tot++;
			}
			
		} else { /* int I/Os */
			/* Enforce interrupt for polled I/O */
			dio->iocb->ki_flags &= ~IOCB_HIPRI;
			bio->bi_opf &= ~REQ_POLLED;
			sc->int_cnt++;
			sc->int_tot++;

			if(sc->int_cnt >= sc->param7) { // 10000) { 
			//if(sc->int_cnt >= sc->N_INT) { 
				sc->mode = 3; // INT -> OL
				sc->ol_cnt = 0;
				//sc->qd = 0;
				sc->qd_sum = 0;
				sc->tf = 0;
				//sc->N_PAS = 1; // pas stat update req.
				//if(sc->ioctr <= 1000)
				//	printk("INT->OL");
			} 
		}
