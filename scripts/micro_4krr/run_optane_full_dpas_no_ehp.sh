...
DEVICES=("nvme1n1")
# Original full-DPAS mode list, kept for reference:
# IO_MODE=("CP" "DPAS" "PAS" "LHP" "INT")
IO_MODE=("PAS" "INT" "CP" "LHP")
JOBS=(20 16 8 4 2 1)
RR_RW=("RR")
REPEATS=(1)
MAX_CORE=20
...
nvme_setup() {
  ...
  if [[ "${NVME_RELOADABLE}" == "1" ]]; then
    ...
    echo "[info] nvme is builtin; skipping modprobe reload"
  fi
}
		    ...
		    if [ ${device} == "nvme1n1" ]; then  # theta := 3 for Optane 5800X (10X)
				...
				THRESHOLD="10" # theta :=1 for NAND SSDs (10X)
                    ...
                    if [ ${mode} == "INT" ]; then
                        ...
                        nvme_setup "${JOBS}"
			# DPAS switch is intentionally disabled for PAS/INT/CP/LHP-only runs.
